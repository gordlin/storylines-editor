...
type possibleSlideshowSlideTypes =
    | ChartEditorV
    | typeof ImageEditorV
    | typeof TextEditorV
    | typeof MapEditorV
...
const slideEditor = ref<possibleSlideshowSlideTypes | null>(null);
...
function saveItem(add = false): void {
    ...
    if (itemConfig.type !== PanelType.Text) {
        if (
            ...
            typeof (slideEditor as unknown as ImageEditorV | ChartEditorV).saveChanges === 'function'
        ) {
            (slideEditor as unknown as ImageEditorV | ChartEditorV).saveChanges();
