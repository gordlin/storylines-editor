...
function configEmpty(index: number, lang: keyof MultiLanguageSlide): boolean {
    const slide: MultiLanguageSlide = props.slides[index];
...
function addNewSlide(): void {
    ...
    selectSlide(props.slides.length - 1, props.lang);
    emit('slides-updated', props.slides);
    emit('scroll-to-element', props.slides.length - 1);
}
...
function copyConfigFromOtherLang(index: number, currLang: keyof MultiLanguageSlide): void {
    ...
    props.slides[index][oppositeLang]?.panel.forEach((panel) => {
    ...
    setTimeout(() => {
        (props.slides[index][currLang] as Slide).panel.forEach((panel) => productStore.removeSourceCounts(panel));
        props.slides[index][currLang] = JSON.parse(JSON.stringify(props.slides[index][oppositeLang]));
        emit('slides-updated', props.slides);
...
function createNewConfig(index: number, currLang: 'en' | 'fr'): void {
    ...
    props.slides[index][currLang]!.panel.forEach((panel: BasePanel) => {
        // Remove source counts for each panel in the current config
        productStore.removeSourceCounts(panel);
    });

    props.slides[index][currLang] = JSON.parse(JSON.stringify(defaultBlankSlide));
...
function copySlide(index: number): void {
    ...
    props.slides[index]?.en?.panel.forEach((panel) => productStore.panelHelper(panel, incrementSourceCounts));
    props.slides[index]?.fr?.panel.forEach((panel) => productStore.panelHelper(panel, incrementSourceCounts));

    // Copy must be created after changes have been saved for the copied slide (via the above call to selectSlide())
    props.slides.splice(index + 1, 0, cloneDeep(props.slides[index]));
    emit('slides-updated', props.slides);
...
function removeSourceCounts(deletedIndex: number): void {
    let panelEn = props.slides.find((slide: MultiLanguageSlide, idx: number) => idx === deletedIndex)?.en?.panel;
    let panelFr = props.slides.find((slide: MultiLanguageSlide, idx: number) => idx === deletedIndex)?.fr?.panel;
...
function moveDown(index: number): void {
    props.slides.splice(index + 1, 0, props.slides.splice(index, 1)[0]);
    emit('scroll-to-element', index + 1);
    emit('slides-updated', props.slides);
}

function getSlideId(slide: MultiLanguageSlide): string {
    return 'slide' + props.slides.indexOf(slide);
}
